...
#define bbColor [UIColor colorWithRed:(26.0/255.0) green:(155.0/255.0) blue:(252.0/255.0) alpha:1.0]
#define BackColor [UIColor colorWithRed:244.0f/255.0f green:244.0f/255.0f blue:244.0f/255.0f alpha:0.0]
// 选中颜色加深
...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        CALayer *bgLayer = [self createBgLayerWithColor:bbColor andPosition:bgLayerPosition];
        [self.layer addSublayer:bgLayer];
        [tempBgLayers addObject:bgLayer];
        //title
        CGPoint titlePosition = CGPointMake( (i * 2 + 1) * textLayerInterval , self.frame.size.height / 2);
        NSString *titleString = [_dataSource menu:self titleForColumn:i];
        CATextLayer *title = [self createTextLayerWithNSString:titleString withColor:[UIColor whiteColor] andPosition:titlePosition];
        [self.layer addSublayer:title];
        [tempTitles addObject:title];
        //indicator
        CAShapeLayer *indicator = [self createIndicatorWithColor:[UIColor whiteColor] andPosition:CGPointMake(titlePosition.x + title.bounds.size.width / 2 + 8, self.frame.size.height / 2)];
        ...
        //separator
         if (i != _numOfMenu - 1) {
             ...
             CAShapeLayer *separator = [self createSeparatorLineWithColor:[UIColor whiteColor] andPosition:separatorPosition];
    ...
    if (leftOrRight == 1) {
        ...
        if (!_hadSelected && _leftSelectedRow == indexPath.row) {
            ...
            if(!haveRightTableView){
                ...
                accessoryImageView.frame = CGRectMake(self.frame.size.width*ratio-40, (self.frame.size.height-12)/2, 16, 12);
